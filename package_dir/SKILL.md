...
- `SET_STRATEGY`: 특정 종목의 매매전략을 저장합니다. Markdown 형식의 본문과 함께 차트 표시 및 자동매매를 위한 구체적인 가격 배열을 포함합니다.
  - Params: `symbol`, `content` (Markdown), `entryPrices?` (`List<number>`), `targetPrices?` (`List<number>`), `stopLoss?` (`number`)
- `GET_STRATEGY`: 저장된 매매전략을 조회합니다.
  - Params: `symbol`

### 4. Trading (실제 매매 주문)
- `BUY_STOCK`: 주식 매수 주문을 실행합니다. (시장가/지정가 지원)
  - Params: `symbol`, `quantity` (number/string), `price?` (지정가용)
- `SELL_STOCK`: 주식 매도 주문을 실행합니다.
  - Params: `symbol`, `quantity`, `price?`
- `ADD_TRADING_RECORD`: 실제 또는 가상의 매매 기록을 저장합니다. (매매복기용)
  - Params: `symbol`, `side` ("BUY"|"SELL"), `price`, `quantity`, `reason`, `date?` (YYYY-MM-DD)

### 5. Account & Watchlist (자산 및 관심종목)
...
```json
{
  ...
  "params": {
    "symbol": "005930",
    "entryPrices": [76000, 77000],
    "targetPrices": [82000, 86000],
    "stopLoss": 74000,
