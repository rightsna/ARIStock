  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        ...
        children: [
          const Icon(Icons.touch_app_outlined, size: 64, color: AppTheme.textMain24),
          const SizedBox(height: 24),
          const Text(
            '분석할 종목을 먼저 선택해 주세요.',
            style: TextStyle(color: AppTheme.textMain54, fontSize: 16, fontWeight: FontWeight.bold),
          ),
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Padding(
        ...
        child: Column(
          children: [
            const Icon(Icons.analytics_outlined, size: 48, color: AppTheme.textMain24),
            const SizedBox(height: 16),
            const Text('아직 통합 분석 내역이 없습니다.', style: TextStyle(color: AppTheme.textMain54)),
            const SizedBox(height: 24),
            if (symbol != null)
              ElevatedButton.icon(
