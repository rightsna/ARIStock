...
class CandleBuilder {
  ...
  static Candle? _parseKiwoomCandle(
    ...
    try {
      final close = double.tryParse(item['stck_clpr']?.toString() ?? '0') ?? 0;
      final open = double.tryParse(item['stck_oprc']?.toString() ?? '0') ?? 0;
      final high = double.tryParse(item['stck_hgpr']?.toString() ?? '0') ?? 0;
      final low = double.tryParse(item['stck_lwpr']?.toString() ?? '0') ?? 0;
      final volume = int.tryParse(item['cntg_vol']?.toString() ?? '0') ?? 0;
      ...
      if (timeframe == 'D' || timeframe == 'W' || timeframe == 'M') {
        ...
        final dateStr = item['stck_bsop_date'] as String? ?? '';
        if (dateStr.length == 8) {
        ...
        final timeStr = item['stck_cntg_hour'] as String? ?? '';
